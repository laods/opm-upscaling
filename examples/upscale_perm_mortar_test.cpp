...
int main(int varnum, char** vararg) {        
    ...
    */
     ...
     *
     * OBS! When using mortar methods to impose periodic BCs, only one 
     * instance of SinglePhaseUpscaler is nescessarry since we don't 
     * require matching grid.
     */
    ...
    */
    ...
    cout << "Tesselating grid ...";
    start = clock();
    upscaler.init(eclParser, SinglePhaseUpscaler::Periodic, minPerm, ztol,  
		  linsolver_tolerance, linsolver_verbosity, linsolver_type, twodim_hack);
    // Not important which BC type we use here, as this will be changed later. 
    // Might be problem with ppartners in z direction?
    finish = clock();
    timeused_tesselation = (double(finish)-double(start))/CLOCKS_PER_SEC;
    cout << " (" << timeused_tesselation << " secs)" << endl << endl;
    ...
    if (eclParser.hasField("PORO")) {
      upscaledPorosity = upscaler.upscalePorosity();
      /*
        if (isPeriodic) {
    ...
    if (isFixed)  {
        ...
        upscaler.setBoundaryConditionType(SinglePhaseUpscaler::Fixed);
	//Kfixed = upscaler_nonperiodic.upscaleSinglePhase();
	Kfixed = upscaler.upscaleSinglePhase();
	...
	upscaler.setBoundaryConditionType(SinglePhaseUpscaler::Linear);
	//Klinear = upscaler_nonperiodic.upscaleSinglePhase();
	Klinear = upscaler.upscaleSinglePhase();
	...
	upscaler.setBoundaryConditionType(SinglePhaseUpscaler::Periodic);
	//Kperiodic = upscaler_periodic.upscaleSinglePhase();
	Kperiodic = upscaler.upscaleSinglePhase();
    ...
    if (isFixed) {
        ...
        outputtmp << "# Tesselation time: " << timeused_tesselation << " s" << endl;
